...
describe('ConfigManager', () => {
  ...
  it('sanitizeIdsでトリムと重複排除が行われる', () => {
    ...
    const result = manager.sanitizeIds([
      '  Alice ',
      'ALICE',
      '',
      'Bob',
  ...
  it('sanitizeWordsで小文字化し重複排除する', () => {
    const manager = new ConfigManager()
    const result = manager.sanitizeWords([
      ' Hello ',
      'hello',
      'WORLD',
      'world '
  ...
  it('parseImportPayloadでstorageKey不一致をエラーにする', () => {
    const manager = new ConfigManager()
    const text = JSON.stringify({
      storageKey: 'other',
      version: CONFIG.EXPORT_VERSION,
    ...
    expect(() => manager.parseImportPayload(text)).toThrow('storageKey')
  })

  it('parseImportPayloadでv1のngWordsを取り込む', () => {
    const manager = new ConfigManager()
    const text = JSON.stringify({
      storageKey: CONFIG.STORAGE_KEY,
      version: 1,
    ...
    const parsed = manager.parseImportPayload(text)
    expect(parsed.textFilterWords).toEqual(['spam'])
  })

  it('loadHiddenUserIdsで文字列保存も整形して読み込む', () => {
    storedHiddenIds = ' Alice ,Bob '
    const manager = new ConfigManager()
    expect(manager.getIds()).toEqual(['Alice', 'Bob'])
  })

  it('upsertHiddenPostIdで期限切れの場合は現在時刻からTTLを再設定する', () => {
    const now = 5000
    storedHiddenPosts = [{ id: 'old', expiresAt: now - 10 }]
    const manager = new ConfigManager()
    manager.upsertHiddenPostId('old', now)
    expect(storedHiddenPosts[0].expiresAt).toBe(now + CONFIG.POST_FILTER.TTL_MS)
  })

  it('isExpiredで期限切れ判定を行う', () => {
    const manager = new ConfigManager()
    expect(manager.isExpired({ expiresAt: 0 }, 1)).toBe(true)
    expect(manager.isExpired({ expiresAt: 2 }, 1)).toBe(false)
  })
})
