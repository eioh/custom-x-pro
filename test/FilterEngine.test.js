...
describe('FilterEngine', () => {
  ...
  it('引用ポストIDは重複を除外する', () => {
    ...
    const cell = buildCell({
      postHrefs: [
        'https://x.com/user/status/111/analytics',
        'https://x.com/user/status/111/analytics'
      ],
      quoteLinkIndexes: [0, 1]
    })
    const engine = new FilterEngine(configManager)
    const { quotedPostIds } = engine.extractPostIds(cell)
    expect(quotedPostIds).toEqual(['111'])
  })

  it('extractUserIdsでwww付きURLも認識する', () => {
    const configManager = {
      hasHiddenPostId: jest.fn(() => false),
      extendHiddenPostExpiry: jest.fn(),
      getIds: jest.fn(() => []),
    ...
    const cell = buildCell({
      userHrefs: ['https://www.x.com/WithWww']
    })
    const engine = new FilterEngine(configManager)
    const { parentUserId } = engine.extractUserIds(cell)
    expect(parentUserId).toBe('WithWww')
  })

  it('引用コンテナが無い場合は本文を親テキストとして扱う', () => {
    const configManager = {
      hasHiddenPostId: jest.fn(() => false),
      extendHiddenPostExpiry: jest.fn(),
      getIds: jest.fn(() => []),
    ...
    const cell = buildCell({
      parentTexts: ['parent text']
    })
    const engine = new FilterEngine(configManager)
    const { parentTexts, quotedTexts } = engine.extractTexts(cell)
    expect(parentTexts).toEqual(['parent text'])
    expect(quotedTexts).toEqual([])
  })
})
